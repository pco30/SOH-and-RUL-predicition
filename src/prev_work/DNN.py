...
from keras.layers import Conv1D, MaxPooling1D, GlobalMaxPooling1D, Dense, Dropout, Reshape, ZeroPadding1D
...
model.summary() 
model.fit(X_train_scaled.reshape(X_train.shape[0], X_train.shape[1], 1), y_train, epochs=400, batch_size=50, verbose=1, validation_data=(X_test_scaled.reshape(X_test.shape[0], X_test.shape[1], 1), y_test)) 
